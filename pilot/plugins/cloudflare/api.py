...
from pilot.plugins.cloudflare.manager import CloudflareTunnelManager
...
@cloudflare_bp.get("/ssh")
def get_cloudflare_ssh_status():
    bench_root = Path(current_app.config["BENCH_ROOT"])
    try:
        config = BenchConfig.read(bench_root)
    except Exception:
    ...
    config_file = Path.home() / ".cloudflared" / f"{config.name}-config.yml"
    if not config_file.exists():
        return jsonify({"enabled": False, "hostname": ""})

    try:
        content = config_file.read_text(encoding="utf-8")
        import re
        pattern = r'-\s+hostname:\s*([^\s\n]+)\s*\n\s*service:\s*ssh://localhost:22'
        match = re.search(pattern, content)
        if match:
            return jsonify({"enabled": True, "hostname": match.group(1)})

        pattern2 = r'-\s+service:\s*ssh://localhost:22\s*\n\s*hostname:\s*([^\s\n]+)'
        match2 = re.search(pattern2, content)
        if match2:
            return jsonify({"enabled": True, "hostname": match2.group(1)})
    except Exception as e:
        return error_response("read_failed", f"Failed to read configuration: {e}", 500)

    return jsonify({"enabled": False, "hostname": ""})


@cloudflare_bp.post("/ssh")
def configure_cloudflare_ssh():
    bench_root = Path(current_app.config["BENCH_ROOT"])
    data = request.get_json(silent=True) or {}
    enable = bool(data.get("enable", False))
    hostname = (data.get("hostname") or "").strip().lower()

    if enable and not hostname:
        return error_response("missing_hostname", "A hostname is required to enable SSH.", 400)
    ...
    try:
        ...
        while i < len(lines):
            line = lines[i]
            if re.match(r'^\s*-\s+hostname:', line) or re.match(r'^\s*-\s+service:', line):
                block_lines = [line]
                j = i + 1
                while j < len(lines) and not re.match(r'^\s*-\s+', lines[j]) and lines[j].strip() != "":
                    block_lines.append(lines[j])
                    j += 1

                block_content = "\n".join(block_lines)
                if "ssh://localhost:22" in block_content:
                    i = j
                    continue
                else:
        ...
        if enable:
            ...
            if "  - service: http_status:404" in content:
                content = content.replace("  - service: http_status:404", f"{new_entry}\n  - service: http_status:404")
            else:
                content += f"\n{new_entry}\n"
        ...
        return error_response("ssh_config_failed", f"Failed to configure SSH access: {e}", 500)
