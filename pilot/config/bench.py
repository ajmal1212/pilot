...
_SECTIONS: tuple[_Section, ...] = (
    ...
    _Section(
        "cloudflare",
        lambda data: CloudflareConfig(**BenchConfig._known_fields(CloudflareConfig, data.get("cloudflare", {}))),
        lambda config: {
            "enabled": config.cloudflare.enabled,
            "tunnel_name": config.cloudflare.tunnel_name,
            "tunnel_token": config.cloudflare.tunnel_token,
            "domain": config.cloudflare.domain,
            "api_token": config.cloudflare.api_token,
        }
        if (
            config.cloudflare.enabled
            or config.cloudflare.tunnel_name
            or config.cloudflare.tunnel_token
            or config.cloudflare.domain
            or config.cloudflare.api_token
        )
        else None,
    ),
)
