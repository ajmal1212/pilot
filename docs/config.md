...
| `jwks_audience` | string | no | `""` | Optional. When set, a remote JWKS token is accepted only if its `aud` claim matches this value — binds tokens to one bench when several share a `jwks_url`. Not inherited by sibling benches. |
